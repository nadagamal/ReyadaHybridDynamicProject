 ...
 */
@property (nonatomic, weak) id<NMPushInboxListDelegate> delegate;
 ...
 *
 *  @param object NMRichPushObject instance, which is after the requested rich push object.
 *
 ...
 *
 *  @param object NMRichPushObject instance, which is before the requested rich push object.
 *
 ...
 * By default fetches notifications having inbox status NMPushInboxStatusRead or NMPushInboxStatusUnread AND having notification type NMNotificationTypeURLPush or NMNotificationTypeRich
 */
- (void)fetchPushInboxList;


/**
 * Fetches notifications with given filter parameters from Netmera.
 *
 * @param types A bit mask specifying the types of the notifications to fetch from Netmera. For a list of values, see NMNotificationType.
 * @param statuses A bit mask specifying the push inbox status of the notifications to fetch from Netmera. For a list of values, see NMPushInboxStatus.
 * @param categories An array specifying categories of notifications to fetch from Netmera. Response returns union of notifications (i.e. if you give @[@"A",@"B"], method will return notifications having category value "A" OR "B").
 * @param handler The block to execute when notification list is fetched from Netmera.
 */
- (void)fetchPushInboxListWithNotificationTypes:(NMNotificationType)types inboxStatus:(NMPushInboxStatus)statuses categoryArray:(NSArray*)categories;

/**
 * Returns number of notifications with given filter parameters in a block handler.
 *
 * @param types A bit mask specifying the types of the notifications to fetch from Netmera. For a list of values, see NMNotificationType.
 * @param statuses A bit mask specifying the push inbox status of the notifications to fetch from Netmera. For a list of values, see NMPushInboxStatus.
 * @param categories An array specifying categories of notifications. Return count is union of notifications (i.e. if you give @[@"A",@"B"], method will return notifications having category value "A" OR "B").
 * @param handler The block to execute when count is fetched from Netmera.
 */
+ (void)fetchPushInboxCountWithNotificationTypes:(NMNotificationType)types inboxStatus:(NMPushInboxStatus)statuses categoryArray:(NSArray*)categories completionHandler:(void(^)(NSInteger count, NSError *error))handler;

/** 
 * Insert the given NMPushObject into the push inbox list.
 *
 *  @param object NMPushObject object to add to the push inbox list.
 *
